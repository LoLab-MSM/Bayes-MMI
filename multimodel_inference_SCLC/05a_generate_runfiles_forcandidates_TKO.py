...
"""
    ...
    if int(number) in dfdict.loc[dfdict.subtype_starting_and_makeup_code.str.contains('\.1.')].index: # contains some topologies without A2 subtype but in that case A2 sampled_params will be removed when building sp_list
...
sampled_params_dict['sp_die_0_NEv2_cell_1'] = norm(loc=np.log10(0.077),scale=.5)"""
    # 'not necessary' comment only written once (above) - no else statement here
    else:
        str_to_write_5 = """"""
    str_to_write_6 = """
