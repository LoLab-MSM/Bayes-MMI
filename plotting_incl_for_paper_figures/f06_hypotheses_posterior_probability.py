...
from helper_functions_and_files.parameter_importance_calculations import get_sumsofweights_allbutstructure
...
for dset in ['TKO','RPM','clA']:
    ...
    for topo in ['0','1.','2','4','5','6','7']:
        ...
        post_prob, prior_prob = get_postprobs_allbutstructure(df, add_to_dict=True)#, which_evidence='BIC_marglik_est') # to look at BIC marginal likelihood estimate
        # to look at sums of weights based on BIC (instead of looking at posterior prob which is above)
        #post_prob = get_sumsofweights_allbutstructure(df, add_to_dict=True, which_IC='BIC')
        tdict[tdict_key] = copy.deepcopy(post_prob)
        pridict[tdict_key] = copy.deepcopy(prior_prob)
    ...
    for topo in ['3','8','9']:
        ...
        post_prob, prior_prob = get_postprobs_allbutstructure(df, add_to_dict=True)#, which_evidence='BIC_marglik_est')
        #post_prob = get_sumsofweights_allbutstructure(df, add_to_dict=True, which_IC='BIC')
        tdict[tdict_key] = copy.deepcopy(post_prob)
        pridict[tdict_key] = copy.deepcopy(prior_prob)
...
make_plots_for_datasets_in_a_row(throughdict,colormap=arrowcolormap,savedir=outdir)
...
# # the whole figure (not shown in the paper)
...
# plt.savefig(outdir+'alltopologies_heatmap.pdf',format='pdf')
# plt.show()
...
plt.setp(ax.xaxis.get_majorticklabels(), ha='right')
fig.subplots_adjust(bottom=0.38,top=0.9,left=0.33,right=0.95)
plt.savefig(outdir+'top3topology_heatmap.pdf',format='pdf')
plt.show()
...
# Y vs A2&Y posterior probabilities (not included in the figures but are in Tables S8-10)
print('divison Y vs A2&Y')
print(tdf[['TKO data structure 2 starts including A','RPM data structure 1. starts including A','SCLC-A cell lines data structure 4 starts including A',]].loc[
    hypo_to_num['div_Yeff'] + hypo_to_num['div_A2Yeff'] + hypo_to_num['div_noeff']]) # Y affects div vs A2&Y affects
print('diff first half Y vs A2&Y')
print(tdf[['TKO data structure 2 starts including A', 'RPM data structure 1. starts including A',
     'SCLC-A cell lines data structure 4 starts including A', ]].loc[
    hypo_to_num['diff_firsthalf_Yeff'] + hypo_to_num['diff_firsthalf_A2Yeff'] + hypo_to_num['diff_firsthalf_noeff']])  # Y affects early transition A2&Y affects
print('diff last half Y vs A2&Y')
print(tdf[['TKO data structure 2 starts including A', 'RPM data structure 1. starts including A',
     'SCLC-A cell lines data structure 4 starts including A', ]].loc[
    hypo_to_num['diff_lasthalf_Yeff'] +  hypo_to_num['diff_lasthalf_A2Yeff'] + hypo_to_num['diff_lasthalf_noeff']])  # Y affects late transition vs A2&Y affects

# Prior probabilities (not included in the figures but are in Tables S8-10)

print('TKO structure 2, starts A +/- others')
for i in pridict['TKO data structure 2 starts including A']:
    if not any([x in i for x in ['AN','NA','NY','YN','NA2','A2N','starts']]):
        print('\t'+i)
        for j in pridict['TKO data structure 2 starts including A'][i]:
            print('\t\t'+j+' : '+str(pridict['TKO data structure 2 starts including A'][i][j]))
print('\n\n')

print('RPM structure 1, starts A +/- others')
for i in pridict['RPM data structure 1. starts including A']:
    if not any([x in i for x in ['AA2','A2A','A2Y','YA2','NA2','A2N','starts','_spec','whichNonNE']]):
        print('\t'+i)
        for j in pridict['RPM data structure 1. starts including A'][i]:
            print('\t\t'+j+' : '+str(pridict['RPM data structure 1. starts including A'][i][j]))
print('\n\n')

print('SCLC-A cell lines structure 4, starts A +/- others')
for i in pridict['SCLC-A cell lines data structure 4 starts including A']:
    if not any([x in i for x in ['NY','YN','A2Y','YA2','AY','YA','starts','_spec','endeff','whichNonNE']]):
        print('\t'+i)
        for j in pridict['SCLC-A cell lines data structure 4 starts including A'][i]:
            print('\t\t'+j+' : '+str(pridict['SCLC-A cell lines data structure 4 starts including A'][i][j]))
print('\n\n')
...
# ##
#
# # x was generated above for this purpose (line 79 ish)
# #x = copy.deepcopy(tdf)
#
...
# #plt.savefig(outdir+'alltopologies_comparingbetweeneffecttype_heatmap.pdf',format='pdf')
# plt.show()
